...
# Reference captures pin culling OFF (#99). Back-face culling is ON by default
# in the viewer, but the assets these references were taken against —
# material_grid.glb, transmission_test.glb, coat_test.glb, diffuse_fuzz_test.glb —
# are generated by scripts/make_material_grid.py, whose uv_sphere() winds its
# triangles CW-from-outside despite the comment claiming CCW. Those spheres are
# therefore inverted relative to glTF, invisible while the renderer was
# two-sided, and wrong the moment anything culls. The .glb files are frozen, so
# the references stay comparable by pinning the renderer instead. Export
# DXR_MODELVIEWER_CULL yourself to override.
( env DXR_MODELVIEWER_DETERMINISTIC=1 DXR_MODELVIEWER_CULL="${DXR_MODELVIEWER_CULL:-0}" $RUN > "$LOG" 2>&1 & )
sleep "$WAIT"
