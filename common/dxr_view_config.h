 ...
 * commit c1e4fe00da0f189122eca14e61e9faaa88e4b38e. It lives in this repo's
 * `common/` (not `openxr_includes/`) because it is an app-side helper, not part
 * of the OpenXR wire-protocol surface `VENDORED.json` pins. The only local
 * delta is this paragraph plus the second-stage macro fallback below. A later
 * displayxr-common release (v2.14.0) ships the same helper; when the demo
 * re-pins to it this copy can go.
 */
...
/*
 * Second stage (local addition). The header WAS found, but an older vendored
 * copy of it (XR_DXR_display_info_SPEC_VERSION < 19) does not define the
 * enumerator yet — this repo's openxr_includes/ is re-vendored by hand, so the
 * two can legitimately be out of step on a branch that bumps only one of them.
 * Narrow by construction: it can only fire when the include above already ran
 * and left the name undefined, so it can never shadow a future Khronos-
 * registered enumerator that the header does declare.
 */
